...
static void hessian
(
  float *G,   //the steepest descent image
...
static void compute_hessian(
  float *GTG, //precomputed matrix G^T G
  float *rho, //robust weights
  float *H,   //output Hessian matrix
  int nparams, //number of parameters
...
(
  float *I,  //second warped image I2(x'(x;p))
  float *Iw, //first image I1(x)
...
(
  ...
  float *rho,   //output robust weights
...
)
{
  ...
  float *Ix =new float[size1];   //x derivate of the first image
  float *Iy =new float[size1];   //y derivate of the first image
  float *J  =new float[size4];   //jacobian matrix for all points
  float *G  =new float[size2];   //steepest descent images
  ...
  float *Iw =new float[size1];   //warp of the second image/
  float *DI =new float[size1];   //error image (I2(w)-I1)
...
)
{
  ...
  #ifdef PRECOMPUTATION_GTG
    //Precompute G^T G
    float *GTG=new float[size5];   //G^T G
  ...
  float *H  =new float[size3];   //Hessian matrix
  float *H_1=new float[size3];   //inverse Hessian matrix
  float *rho=new float[size0];   //robust function
...
)
{
    ...
    for(int s=1; s<nscales; s++)
    {
      ...
      I1s[s]=new float[size*nzz];
      I2s[s]=new float[size*nzz];
